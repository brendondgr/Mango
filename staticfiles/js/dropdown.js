...
function toggleLLMDropdown() {
    console.log('Toggling LLM Dropdown');
...
function toggleSidePanel() {
    ...
    if (sidePanel.classList.contains('w-0')) {
        ...
        sidePanel.classList.remove('border-slate-700/0');
        sidePanel.classList.add('border-slate-700');
        loadSidePanelData();
    } else {
        sidePanel.classList.add('w-0');
        sidePanel.classList.remove('w-80');
        sidePanel.classList.add('border-slate-700/0');
        sidePanel.classList.remove('border-slate-700');
    }
}
...
function renderModelList(type, models) {
    ...
    models.forEach(model => {
        ...
        const statusColor = model.exists ? 'bg-green-500' : 'bg-red-500';
        ...
        div.innerHTML = `
            <div class="flex items-center gap-2 overflow-hidden">
                ...
                <div class="flex-1 min-w-0">
                    <div class="text-xs font-medium text-slate-200 truncate" title="${model.nickname}">${model.nickname}</div>
                    <div class="text-[10px] text-slate-500 truncate" title="${model.file_name}">${model.file_name}</div>
                </div>
            </div>
            <button onclick="removeModel('${type}', '${model.file_name}')" class="text-slate-400 hover:text-red-400 ml-2">
